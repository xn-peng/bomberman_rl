...
def setup(self):
    ...
    self.model = DQN(15, 15, 6).to(device)
    if self.train or not os.path.isfile("my-saved-model43.tar"):
...
def act(self, game_state: dict) -> str:
    ...
    if self.train:
        ...
        if random.random() < self.exploration_rate:
            ...
            return np.random.choice(ACTIONS, p=[.2, .2, .2, .2, .1, .1])#p=[.22, .22, .22, .22, .11, 0.01])
        # make policy net choose an action
        else:
            with torch.no_grad():
                myprobs = \
                self.policy_net(torch.Tensor(state_to_features(game_state)).view(1, 1, 15, 15).to(device)).detach()[0]
            self.all_actions[torch.argmax(myprobs).item()] += 1
            self.Q_list.append(torch.max(myprobs).item())
            return ACTIONS[torch.argmax(myprobs).item()]

    else:
        # choose only best action when not training

        with torch.no_grad():
            myprobs = self.model(torch.Tensor(state_to_features(game_state)).view(1, 1, 15, 15).to(device)).detach()[0]
        # myprobs = myprobs
        # print(myprobs)
        return ACTIONS[torch.argmax(myprobs).item()]
