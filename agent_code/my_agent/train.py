...
GAMMA = 0.95
TAU = 1e-4  # update rate for target netwsork
...
def setup_training(self):
    ...
    self.target_net = DQN(15, 15, 6).to(device)
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    # do the optimization
    if self.total_steps > 10000 and self.total_steps%UPDATE_EVERY==0:
        ...
        action_batch = torch.from_numpy(np.asarray(batch.action)).view(RANDOM_BATCH_SIZE, 1).to(device)
        reward_batch = torch.from_numpy(np.asarray(batch.reward)).float().to(device)

        # get Q values
        state_action_value = self.policy_net(state_batch.view(RANDOM_BATCH_SIZE,1,15,15)).gather(1, action_batch.type(torch.int64))
        next_state_value = torch.zeros(RANDOM_BATCH_SIZE).to(device)
...
for key in game_rewards:
    game_rewards[key] = game_rewards[key]/10
...
def save_model(self):
    ...
    if self.total_steps%50000 == 0:
        ...
        'target_state_dict': self.target_net.state_dict(),
        'optimizer_state_dict': self.optimizer.state_dict(),
        }, 'my-saved-model{}.tar'.format(self.save_index))
